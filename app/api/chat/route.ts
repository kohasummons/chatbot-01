...
function sanitizeUserInput(content: string): string {
  ...
  content = content.replace(/system:|as system:|system message:|<system>|assistant:|\bas\s+the\s+system\b/gi, "[FILTERED]");
  
  // Remove any attempts to override or ignore instructions
  content = content.replace(/ignore previous instructions|ignore your instructions|new instructions|disregard|forget/gi, "[FILTERED]");
...
function validateChatMessage(message: any): boolean {
  ...
  if (!['system', 'user', 'assistant', 'tool', 'function'].includes(message.role)) return false;
      ...
      tools: {
        ...
        book_appointment: tool({
          ...
          parameters: z.object({
            ...
            reason: z.enum(['Checkup', 'Emergency', 'Filling']).default('Checkup').describe('The reason for the appointment')
          }),
          execute: async ({ patient_name, date, time, dentist_id, patient_email, reason }) => {
            try {
